...
> This lecture is part of [The Missing Semester of Your CS Education](https://missing.csail.mit.edu/2020/data-wrangling/), MIT. Licensed under [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/).

<iframe width="560" height="315" src="https://www.youtube.com/embed/sz_dsktIjt4" frameborder="0" allowfullscreen></iframe>
...
`BEGIN` is a pattern that matches the start of the input (and `END` matches the end). Now, the per-line block just adds the count from the first field (although it'll always be 1 in this case), and then we print it out at the end. In fact, we *could* get rid of `grep` and `sed` entirely, because `awk` [can do it all](https://web.archive.org/web/20251210045942/https://backreference.org/2010/02/10/idiomatic-awk/), but we'll leave that as an exercise to the reader.
