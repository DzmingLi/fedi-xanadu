//! Course homeworks as a first-class entity. Replaces the JSONB-stuffed
//! `type='hw'` entries that used to live inside `course_sessions.resources`
...
pub async fn create_homework(
    ...
    created_by: &str,
) -> crate::Result<CourseHomework> {
    let id = format!("chw-{}", tid());
    sqlx::query(
        "INSERT INTO course_homeworks \
            (id, course_id, session_id, label, url, description, position, due_date, created_by) \
...
pub async fn get_homework(pool: &PgPool, id: &str) -> crate::Result<CourseHomework> {
    sqlx::query_as::<_, CourseHomework>(
        "SELECT id, course_id, session_id, label, url, description, position, due_date, \
                created_by, created_at \
         FROM course_homeworks WHERE id = $1",
    )
    .bind(id)
    .fetch_optional(pool)
    .await?
    .ok_or_else(|| crate::Error::NotFound { entity: "course_homework", id: id.to_string() })
}

pub async fn list_homeworks_by_course(pool: &PgPool, course_id: &str) -> crate::Result<Vec<CourseHomework>> {
    Ok(sqlx::query_as::<_, CourseHomework>(
        "SELECT id, course_id, session_id, label, url, description, position, due_date, \
                created_by, created_at \
         FROM course_homeworks WHERE course_id = $1 \
...
pub async fn list_homeworks_by_session(pool: &PgPool, session_id: &str) -> crate::Result<Vec<CourseHomework>> {
    Ok(sqlx::query_as::<_, CourseHomework>(
        "SELECT id, course_id, session_id, label, url, description, position, due_date, \
                created_by, created_at \
         FROM course_homeworks WHERE session_id = $1 \
         ORDER BY position, created_at",
    )
    .bind(session_id)
    .fetch_all(pool)
...
pub async fn delete_homework(pool: &PgPool, id: &str) -> crate::Result<()> {
    let res = sqlx::query("DELETE FROM course_homeworks WHERE id = $1")
        .bind(id)
        .execute(pool)
        .await?;
    if res.rows_affected() == 0 {
        return Err(crate::Error::NotFound { entity: "course_homework", id: id.to_string() });
    }
    Ok(())
}
