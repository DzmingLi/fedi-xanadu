...
use fx_core::services::term_service::{self, TermRow, TermListRow, TermSessionRow, TermDetailResponse, TermRatingStats, TermReviewRow, CreateTerm, UpdateTerm, CreateSession, UpdateSession};
...
) -> ApiResult<Json<Vec<TermListRow>>> {
    let terms = term_service::list_terms(&state.pool).await?;
    Ok(Json(terms))
}

pub async fn my_terms(
    State(state): State<AppState>,
    WriteAuth(user): WriteAuth,
) -> ApiResult<Json<Vec<TermListRow>>> {
    let terms = term_service::list_my_terms(&state.pool, &user.did).await?;
    Ok(Json(terms))
}
...
pub async fn get_term(
    ...
    Path(id): Path<String>,
) -> ApiResult<Json<TermDetailResponse>> {
    let viewer = user.as_ref().map(|u| u.did.as_str());
    let detail = term_service::get_term_detail(&state.pool, &id, viewer).await?;
    Ok(Json(detail))
}
...
) -> ApiResult<(StatusCode, Json<TermRow>)> {
    if let Err(e) = fx_core::validation::validate_title(&input.title) {
        return Err(AppError(fx_core::Error::Validation(vec![e])));
    }

    let id = format!("trm-{}", tid());
    let term = term_service::create_term(&state.pool, &id, &user.did, &input).await?;
...
) -> ApiResult<Json<TermRow>> {
    let summary = input.summary.as_deref().unwrap_or("");
    let term = term_service::update_term(&state.pool, &id, &user.did, &input, summary).await?;
    Ok(Json(term))
}
...
pub async fn delete_term(
    ...
    term_service::delete_term(&state.pool, &id, &user.did).await?;
...
pub async fn remove_series(
    ...
    term_service::remove_series(&state.pool, &id, series_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn add_skill_tree(
    ...
    term_service::add_skill_tree(&state.pool, &id, &input.tree_uri, input.role.as_deref().unwrap_or("prerequisites")).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn list_patches(
    ...
    let patches = patch_service::list_patches(&state.pool, "term", &id).await?;
    Ok(Json(patches))
}
...
pub async fn add_tag(
    ...
    term_service::add_tag(&state.pool, &id, &input.tag_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn remove_tag(
    ...
    term_service::remove_tag(&state.pool, &id, tag_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn add_textbook(
    ...
    term_service::add_textbook(&state.pool, &id, &input.book_id, input.role.as_deref().unwrap_or("required"), input.sort_order.unwrap_or(0)).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn remove_textbook(
    ...
    term_service::remove_textbook(&state.pool, &id, book_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
) -> ApiResult<(StatusCode, Json<TermSessionRow>)> {
    let session_id = format!("csn-{}", tid());
    let session = term_service::create_session(&state.pool, &session_id, &term_id, &input).await?;
...
pub async fn update_session(
    State(state): State<AppState>,
    WriteAuth(_user): WriteAuth,
    Path((_term_id, session_id)): Path<(String, String)>,
    Json(input): Json<UpdateSession>,
) -> ApiResult<Json<TermSessionRow>> {
    let session = term_service::update_session(&state.pool, &session_id, &input).await?;
    Ok(Json(session))
}
...
pub async fn delete_session(
    ...
    term_service::delete_session(&state.pool, &session_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn add_session_tag(
    ...
    fx_core::services::tag_service::require_tag_id(&input.tag_id)?;
    term_service::add_session_tag(&state.pool, &session_id, &input.tag_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn remove_session_tag(
    ...
    let tag_id = q.get("tag_id").ok_or(AppError(fx_core::Error::BadRequest("missing tag_id".into())))?;
    fx_core::services::tag_service::require_tag_id(tag_id)?;
    term_service::remove_session_tag(&state.pool, &session_id, tag_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn add_session_prereq(
    ...
    fx_core::services::tag_service::require_tag_id(&input.tag_id)?;
    term_service::add_session_prereq(&state.pool, &session_id, &input.tag_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn remove_session_prereq(
    ...
    let tag_id = q.get("tag_id").ok_or(AppError(fx_core::Error::BadRequest("missing tag_id".into())))?;
    fx_core::services::tag_service::require_tag_id(tag_id)?;
    term_service::remove_session_prereq(&state.pool, &session_id, tag_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
#[derive(Deserialize)]
pub struct RateTermInput {
    rating: i16,
}

pub async fn rate_term(
    State(state): State<AppState>,
    WriteAuth(user): WriteAuth,
    Path(id): Path<String>,
    Json(input): Json<RateTermInput>,
) -> ApiResult<Json<TermRatingStats>> {
    if input.rating < 1 || input.rating > 10 {
        return Err(AppError(fx_core::Error::BadRequest("rating must be 1-10".into())));
    }
    let stats = term_service::rate_term(&state.pool, &id, &user.did, input.rating).await?;
...
pub async fn unrate_term(
    State(state): State<AppState>,
    WriteAuth(user): WriteAuth,
    Path(id): Path<String>,
) -> ApiResult<Json<TermRatingStats>> {
    let stats = term_service::unrate_term(&state.pool, &id, &user.did).await?;
...
#[derive(Deserialize)]
pub struct AddTermResourceInput {
    pub kind: String,
    pub label: String,
    pub url: String,
    #[serde(default)]
...
pub async fn list_resources(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> ApiResult<Json<Vec<term_service::TermResource>>> {
    let rows = term_service::list_term_resources(&state.pool, &id).await?;
    Ok(Json(rows))
}
...
pub async fn add_resource(
    ...
    WriteAuth(user): WriteAuth,
    Path(id): Path<String>,
    Json(input): Json<AddTermResourceInput>,
    ...
    let new_id = term_service::add_term_resource(
...
pub async fn delete_resource(
    ...
    term_service::delete_term_resource(&state.pool, &resource_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn set_learning_status(
...
) -> ApiResult<Json<term_service::TermLearningStatus>> {
    ...
    let row = term_service::set_learning_status(&state.pool, &id, &user.did, &input.status).await?;
    Ok(Json(row))
}
...
pub async fn remove_learning_status(
    ...
    term_service::remove_learning_status(&state.pool, &id, &user.did).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn set_session_progress(
...
) -> ApiResult<Json<Option<term_service::TermLearningStatus>>> {
    let status = term_service::set_session_progress(
    ...
    if input.completed {
        let tag_ids: Vec<String> = sqlx::query_scalar(
            "SELECT tag_id FROM term_session_tags WHERE session_id = $1",
...
pub async fn get_reviews(
    ...
    let items = term_service::list_term_articles_by_category(&state.pool, &id, "review", limit, offset).await?;
    let total = term_service::count_term_articles_by_category(&state.pool, &id, "review").await?;
...
pub async fn get_notes(
    ...
    let limit = q.limit.unwrap_or(30).clamp(1, 200);
    let offset = q.offset.unwrap_or(0).max(0);
    let items = term_service::list_term_articles_by_category(&state.pool, &id, "note", limit, offset).await?;
    let total = term_service::count_term_articles_by_category(&state.pool, &id, "note").await?;
    Ok(Json(PagedReviews { items, total }))
}
...
pub async fn get_discussions(
    ...
    let items = fx_core::services::comment_service::list_top_comments(&state.pool, &term_uri, limit, offset).await?;
    let total = fx_core::services::comment_service::count_top_comments(&state.pool, &term_uri).await?;
