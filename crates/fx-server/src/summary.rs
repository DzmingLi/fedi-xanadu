//! Summary (abstract) rendering helper.
//!
//! Article/series summaries are authored in the same format as the content
//! (markdown or typst) and rendered to inline-only HTML for card previews and
...
/// Render summary source to inline-only HTML: full render through
/// `fx_renderer`, then strip block-level wrappers so the result is safe to
/// embed in a card's `<p>` without breaking layout.
pub fn render_summary_inline(format: &str, source: &str, repo_path: &Path) -> Result<String> {
...
mod tests {
    ...
    fn empty_in_empty_out() {
        ...
        let got = render_summary_inline("markdown", "", p).unwrap();
        assert_eq!(got, "");
    }
}
