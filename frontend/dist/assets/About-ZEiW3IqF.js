import{p as i,k as t,l as r,v as d}from"./index-DqN-n4tv.js";import"./legacy-Cymc5jnl.js";import{h as s}from"./html-P3znp1QD.js";import{g as p,i as k}from"./marked.esm-CnO_dgo5.js";const u=`# Fedi-Xanadu：让知识流向对的人
...
### 3. 问答：知乎和 Stack Overflow 解决不了的那些问题

问答功能和文章共享同一套底层——一个问题是一篇 \`kind = question\` 的文章，一个回答是 \`kind = answer\` 的文章。这意味着问答天然继承了前置知识匹配：

- **你只会看到你能看懂的问题**。一个关于 Dependent Type Theory 的问题不会出现在刚入门编程的人的首页——不是因为算法猜你不感兴趣，而是因为你还没点亮前置 Tag
- **回答支持 Typst**。在知乎上写一个稍复杂的数学推导是折磨，在 Stack Overflow 上更是。在这里，一个带完整推导过程的答案和一篇论文没有区别
- **答案可以被 Fork**。觉得某个回答接近完美但有一个地方可以说得更清楚？Fork 它，改掉，你的版本会出现在原答案旁边。社区投票决定哪个版本更好，而不是只有一个"被接受的答案"
- **你的问题和回答都在你的 PDS 里**。知乎可以删除你的回答，Stack Overflow 可以封禁你的账号。在这里，你的数据是你的

这解决了一个知乎和 Stack Overflow 共同的根本缺陷：**它们的算法分发不考虑你是否有能力理解这个问题**。一个关于 GHC 内存模型的问题被推给了五千个 Python 初学者，零有效回答——这是平台架构的失败，不是提问者的错。

### 4. 图书：豆瓣和 Goodreads 缺少的那一层

图书是平台上的一等公民，而不是文章里随手贴的一个外链。每本书有完整的元数据：作者、简介、封面，以及**多个版本**——因为对学术书来说，哪个译者翻译的、哪个出版社出的、哪一年的版次，是真正重要的信息。《计算机程序的构造与解释》的裘宗燕译本和另一个版本不是同一本书。

图书与技能树深度整合：

- **一本书可以声明它教什么 Tag**。《TAPL》教 Type Theory、Lambda Calculus、Subtyping；系统据此把它推给正在学这些主题的人，而不是推给所有人
- **书评是一篇完整的文章**，可以用 Typst 写，可以有数学推导，可以被 Fork 和讨论。不是豆瓣的几颗星和一段短评，是真正的学术书评
- **书评可以绑定到具体版本**。你对某个译本的评价和对原版的评价是分开的，不会混在一起
- **阅读进度精确到章节**。标记"想读 / 在读 / 读完"之外，还可以追踪每一章的完成状态
- **书籍元数据由社区 wiki 式维护**，所有修改都有记录，像维基百科一样可以回溯，任何人都可以纠错

豆瓣的书库是封闭的，Goodreads 的书评被亚马逊拥有。在这里，书籍数据库是开放的，你的书单和书评在你的 PDS 里。
...
- **知乎/Stack Overflow**：中心化平台，算法分发不考虑前置知识，数学公式体验差，回答被平台拥有
- **豆瓣/Goodreads**：书评与知识图谱割裂，无前置知识匹配，书评被平台拥有，版本管理粗糙
...
- **开源**：AGPL-3.0
`;var l=d('<div class="content svelte-8kinj7"></div>');function T(a,o){i(o,!1);const e=p.parse(u);k();var n=l();s(n,()=>e,!0),t(a,n),r()}export{T as default};
