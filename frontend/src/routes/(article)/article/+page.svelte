<script lang="ts">
  ...
  $effect(() => {
    if ($articleQuery.error) {
      error = ($articleQuery.error as Error).message;
    }
  });
  ...
  function doFork() {
    if (!article) return;
    goto(`/new?fork_of=${encodeURIComponent(article.at_uri)}`);
  }

  function doEdit() {
    if (!article) return;
    goto(`/new?edit=${encodeURIComponent(article.at_uri)}`);
  }
