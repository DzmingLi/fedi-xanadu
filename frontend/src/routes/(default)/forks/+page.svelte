<script lang="ts">
  ...
  const articleQuery = createQuery({
    queryKey: () => keys.articles.full(uri),
    queryFn: () => getArticle(uri),
    enabled: () => !!uri,
  });

  const forksQuery = createQuery({
    queryKey: () => ['articles', 'forks', uri] as const,
    queryFn: () => getArticleForks(uri),
    enabled: () => !!uri,
  });

  let article = $derived($articleQuery.data ?? null);
  let forks = $derived($forksQuery.data ?? []);
  let loading = $derived($articleQuery.isPending || $forksQuery.isPending);
