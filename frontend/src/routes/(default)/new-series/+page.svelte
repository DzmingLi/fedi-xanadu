<script lang="ts">
  ...
  let allArticles = $derived($articlesQuery.data ?? []);
  let allSeries = $derived($seriesQuery.data ?? []);

  // Initialize parentId from URL param once data loads
  $effect(() => {
    if (parentId && allSeries.length > 0 && !selectedParentId) {
      selectedParentId = parentId;
      const parent = allSeries.find(s => s.id === parentId);
  ...
  async function submit() {
    ...
    try {
      ...
      goto(`/series?id=${encodeURIComponent(series.id)}`);
    } catch (e: any) {
