<script lang="ts">
  ...
  let forkOf = $derived($page.url.searchParams.get('fork_of') ?? '');
  let editUri = $derived($page.url.searchParams.get('edit') ?? '');
  let initialDraftId = $derived($page.url.searchParams.get('draftId') ?? '');
  let initialCategory = $derived($page.url.searchParams.get('category') ?? '');
  let initialBookId = $derived($page.url.searchParams.get('book_id') ?? '');
  ...
  // Sync initialDraftId to currentDraftId
  $effect(() => { currentDraftId = initialDraftId; });
  ...
  // Sync initial params to state
  $effect(() => { category = (initialCategory || 'general') as Category; });
  $effect(() => { bookId = initialBookId || ''; });
  ...
  async function submit() {
    ...
    try {
      if (isEditing && editUri) {
        ...
        goto(`/article?uri=${encodeURIComponent(article.at_uri)}`);
      } else {
        const article = await createArticle({
          title: title.trim(),
          description: description.trim() || undefined,
