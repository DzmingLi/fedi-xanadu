<script lang="ts">
  ...
  let questionQuery = createQuery({
    queryKey: () => keys.questions.byUri(uri),
    queryFn: () => getQuestionDetail(uri),
    enabled: () => !!uri,
  });

  let detail = $derived($questionQuery.data ?? null);
  let loading = $derived($questionQuery.isPending);
  let error = $derived($questionQuery.error?.message ?? '');

  // Content loading via effects (depends on detail)
  let questionContent = $state<ArticleContent | null>(null);
  let answerContents = $state(new Map<string, ArticleContent>());

  $effect(() => {
    if (detail) {
      getArticleContent(uri).then(c => { questionContent = c; }).catch(() => {});
