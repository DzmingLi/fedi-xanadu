<script lang="ts">
  ...
  const teachesQuery = createQuery({
    queryKey: keys.articles.teaches,
    queryFn: () => getAllArticleTeaches(),
  });

  let questions = $derived($questionsQuery.data ?? []);
  let articleTeaches = $derived(buildArticleRowMap($teachesQuery.data ?? []));
  let loading = $derived($questionsQuery.isPending || $teachesQuery.isPending);
