<script lang="ts">
  ...
  const items: RoadmapItem[] = [
    ...
    { title: { en: 'Markdown + MathML', zh: 'Markdown + MathML 数学渲染', fr: 'Markdown + MathML' }, desc: { en: 'Write articles in Markdown; LaTeX math formulas render as native MathML (no JS dependency)', zh: '支持 Markdown 写作，LaTeX 数学公式渲染为原生 MathML（无 JS 依赖）', fr: 'Rédiger en Markdown avec rendu natif MathML (sans dépendance JS)' }, status: 'done' },
    { title: { en: 'Series virtual-document rendering', zh: '系列虚拟文档渲染', fr: 'Rendu de document virtuel pour les séries' }, desc: { en: 'Cross-chapter references resolve automatically — series chapters are compiled as a single document', zh: '跨章节引用自动解析——系列章节作为一个文档整体编译', fr: 'Les références inter-chapitres sont résolues automatiquement' }, status: 'done' },
    ...
    // Planned
    { title: { en: 'Native Typst MathML', zh: 'Typst 原生 MathML 支持', fr: 'MathML natif Typst' }, desc: { en: 'When Typst adds native MathML output, we will adopt it directly — replacing the current mathyml shim with zero-overhead native rendering', zh: '当 Typst 原生支持 MathML 输出时，将直接采用，替换当前的 mathyml 中间层，实现零开销原生渲染', fr: 'Lorsque Typst ajoutera la sortie MathML native, nous l\'adopterons directement — remplaçant le shim mathyml actuel' }, status: 'planned' },
