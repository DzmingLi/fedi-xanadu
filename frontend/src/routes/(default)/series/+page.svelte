<script lang="ts">
  ...
  let seriesQuery = createQuery({
    queryKey: () => keys.series.byId(id),
    queryFn: () => getSeries(id),
    enabled: () => !!id,
  });

  let detail = $derived($seriesQuery.data ?? null);
  let loading = $derived($seriesQuery.isPending);
  let error = $derived($seriesQuery.error?.message ?? '');
  ...
  // Load tree, votes, bookmarks when detail changes
  $effect(() => {
    if (!detail) return;
    const d = detail;
