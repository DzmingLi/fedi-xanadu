<script lang="ts">
  ...
  let foldersQuery = createQuery({
    queryKey: keys.bookmarks.folders,
    queryFn: () => listBookmarkFolders(),
    enabled: () => !!getAuth(),
  });

  let keybindingsQuery = createQuery({
    queryKey: keys.keybindings.all,
    queryFn: () => getKeybindings(),
    enabled: () => !!getAuth(),
  });

  let blocksQuery = createQuery({
    queryKey: keys.blocks.all,
  ...
  let allFolders = $derived($foldersQuery.data ?? []);
  let loading = $derived($settingsQuery.isPending);

  // Sync form state when settings data arrives
  $effect(() => {
  ...
  $effect(() => {
    const kb = $keybindingsQuery.data;
    if (kb) {
      bindings = { ...kb.bindings };
    } else if ($keybindingsQuery.isError) {
      bindings = getAllBindings();
    }
  });

  $effect(() => {
    if ($blocksQuery.data) blockedUsers = $blocksQuery.data;
  });

  $effect(() => {
    if ($membersQuery.data) members = $membersQuery.data;
  });
