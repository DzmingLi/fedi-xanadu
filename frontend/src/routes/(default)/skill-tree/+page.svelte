<script lang="ts">
  ...
  let skillTreeQuery = createQuery({
    queryKey: () => keys.skillTrees.byUri(uri),
    queryFn: () => getSkillTree(uri),
    enabled: () => !!uri,
  });
  ...
  let detail = $derived($skillTreeQuery.data ?? null);
  let loading = $derived($skillTreeQuery.isPending);
  ...
  async function doFork() {
    ...
    goto(`/skill-tree?uri=${encodeURIComponent(result.at_uri)}`);
  }
