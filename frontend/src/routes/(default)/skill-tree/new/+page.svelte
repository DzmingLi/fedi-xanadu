<script lang="ts">
  ...
  async function submit() {
    ...
    try {
      ...
      goto(`/skill-tree?uri=${encodeURIComponent(tree.at_uri)}`);
    } catch (e: any) {
...
<div class="form">
  ...
  <label>
    ...
    <div class="field-input-wrap">
      ...
      {#if tagId}
        <button class="field-clear" onclick={clearField} type="button">x</button>
      {/if}
    ...
    <div class="edge-list">
      {#each edges as e, i}
        <div class="edge-row">
          ...
          <button class="remove-btn" onclick={() => removeEdge(i)}>x</button>
        </div>
      {/each}
    </div>
  {/if}
