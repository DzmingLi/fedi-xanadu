<script lang="ts">
  ...
  import { listSkillTrees, adoptSkillTree, castVote } from '$lib/api';
  ...
  const treesQuery = createQuery({
    queryKey: keys.skillTrees.all,
    queryFn: () => listSkillTrees(),
  });

  let trees = $derived($treesQuery.data ?? []);
  let loading = $derived($treesQuery.isPending);
