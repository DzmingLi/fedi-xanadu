<script lang="ts">
  ...
  let graphQuery = createQuery({
    queryKey: keys.graph.all,
    queryFn: () => getGraph(),
  });

  let treeQuery = createQuery({
    queryKey: keys.tags.tree,
    queryFn: () => getTagTree(),
  });

  let loading = $derived($graphQuery.isPending || $treeQuery.isPending);
  let nodes = $derived($graphQuery.data?.nodes ?? []);
  let edges = $derived($graphQuery.data?.edges ?? []);
  let tree = $derived($treeQuery.data ?? []);
