<script lang="ts">
  ...
  async function submit() {
    ...
    try {
      ...
      navigate(`/term?id=${encodeURIComponent(term.id)}`);
    } catch (e: any) {
      error = e.message;
    }
    submitting = false;
...
{#if !getAuth()}
  <p class="meta">Please log in to create a term.</p>
{:else}
  <div class="new-term">
    <h1>Create Term</h1>
    ...
    <div class="form-field">
      ...
      <textarea bind:value={description} rows="3" placeholder="Brief term description..."></textarea>
    </div>
