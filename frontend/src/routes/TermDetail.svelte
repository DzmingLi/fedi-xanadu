<script lang="ts">
  ...
  let isOwner = $derived(detail && getAuth()?.did === detail.term.did);
  ...
  async function submitAddTag(fromSuggestion?: string) {
    ...
    tagSaving = true;
    try {
      await addTermTag(detail.term.id, id);
      // Refresh so tag_name comes back from server
      detail = await getTermDetail(detail.term.id);
      tagInput = '';
      tagSuggestions = [];
  ...
  async function submitRemoveTag(tagId: string) {
    if (!detail) return;
    try {
      await removeTermTag(detail.term.id, tagId);
      detail = await getTermDetail(detail.term.id);
    } catch (e: any) { tagError = e.message ?? String(e); }
  }
  ...
  $effect(() => {
    ...
    getTermDetail(id)
      .then(d => {
        ...
        document.title = `${d.term.title} — NightBoat`;
      })
  ...
  async function submitRating(value: number) {
    myRating = value;
    const stats = await rateTerm(id, value);
    avgRating = stats.avg_rating;
    ratingCount = stats.rating_count;
  }

  async function clearRating() {
    const stats = await unrateTerm(id);
    myRating = 0;
    avgRating = stats.avg_rating;
    ratingCount = stats.rating_count;
  }
  ...
  async function setStatus(status: 'want_to_learn' | 'learning' | 'finished' | 'dropped') {
    if (learningStatus === status) {
      ...
      learningStatus = '';
      learningProgress = 0;
      try { await removeTermLearningStatus(id); } catch { /* */ }
    } else {
      try {
        const row = await setTermLearningStatus(id, status);
  ...
  async function saveSessionEdit() {
    ...
    try {
      ...
      if (sessionEditId) {
        await updateTermSession(id, sessionEditId, payload);
      } else {
        await createTermSession(id, payload);
      }
      showSessionEdit = false;
      const d = await getTermDetail(id);
      detail = d;
    } catch (e: any) {
  ...
  async function confirmDeleteSession(sessionId: string) {
    ...
    try {
      await deleteTermSession(id, sessionId);
      const d = await getTermDetail(id);
      detail = d;
    } catch (e: any) {
  ...
  <div class="term-page">
    <div class="term-header">
      <div class="header-main">
        {#if c.code}
          <span class="term-code">{c.code}</span>
        {/if}
        <h1 class="term-title">{c.title}</h1>
        {#if c.institution || c.department}
          <p class="term-institution">
            {c.institution || ''}{#if c.institution && c.department} — {/if}{c.department || ''}
          </p>
        {/if}
        {#if c.semester}
          <p class="term-semester">{c.semester}</p>
        {/if}
        {#if detail.siblings.length > 0}
          <div class="term-siblings" aria-label={t('term.otherIterations')}>
            <span class="siblings-label">{t('term.otherIterations')}:</span>
            {#each detail.siblings as s (s.id)}
              <a class="sibling-chip" href="/term?id={encodeURIComponent(s.id)}">
                {s.semester || s.title}
              </a>
            {/each}
          </div>
        ...
        {/if}
        {#if c.description}
          <p class="term-desc">{c.description}</p>
        {/if}
        <div class="term-tags">
          {#each detail.tags as tag}
            <span class="term-tag">
              ...
              {#if isOwner}
                <button class="tag-rm" title={t('term.removeTag')} onclick={() => submitRemoveTag(tag.tag_id)}>×</button>
              {/if}
            </span>
          {/each}
          {#if isOwner}
            <div class="term-tag-add">
              <input
                class="tag-add-input"
                bind:value={tagInput}
                placeholder={t('term.addTagPlaceholder')}
          ...
          {#if (learningStatus === 'learning' || learningStatus === 'dropped') && totalSessions > 0}
            <div class="progress-section">
              <div class="progress-readout">
                <span>{t('term.progress')}: {doneSessions} / {totalSessions} {t('term.sessionsDone')}</span>
      ...
      <div class="header-side">
        {#if isOwner}
          <a href="/new-term?edit={encodeURIComponent(c.id)}" class="edit-btn">{t('common.edit')}</a>
        {/if}
        <div class="term-meta-box">
          <span class="meta-item">{t('term.license')}: {c.license}</span>
          <span class="meta-item">{t('term.language')}: {c.lang.toUpperCase()}</span>
        </div>
      </div>
    </div>
      ...
      <div class="body-side">
        ...
        {#if detail.prerequisites.length > 0}
          <section class="prereqs">
            <h3>{t('term.prerequisites')}</h3>
            {#each detail.prerequisites as p}
              <a href="/term?id={encodeURIComponent(p.prereq_term_id)}" class="prereq-link">
        ...
        <section class="qa-summary">
          ...
          {#if detail.discussions.length === 0}
            <p class="qa-empty">{t('term.noDiscussions')}</p>
          {:else}
            {#each detail.discussions as c}
              <a href="/term-discussions?id={encodeURIComponent(id)}#c-{c.id}" class="qa-item" title={c.title || c.body}>
            ...
            {#if detail.discussion_count > detail.discussions.length}
              <a href="/term-discussions?id={encodeURIComponent(id)}" class="qa-more">{t('term.viewAll')} ({detail.discussion_count}) →</a>
            {/if}
          {/if}
          {#if getAuth()}
            <a href="/term-discussions?id={encodeURIComponent(id)}#new" class="qa-ask-btn">{t('term.askQuestion')}</a>
          {/if}
        </section>
      </div>
    </div>

    <!-- Reviews (opinions on the term) -->
    <section class="reviews-section">
      <h2>{t('term.reviews')}</h2>
      {#if getAuth()}
        <a href="/new?category=review&term_id={encodeURIComponent(id)}" class="write-review-btn">{t('term.writeReview')}</a>
      {/if}
      ...
      {:else}
        {#each detail.reviews as review}
          <a href="/article?uri={encodeURIComponent(review.at_uri)}" class="review-card">
            <div class="review-header">
              ...
              {#if review.term_session_id}
                {@const lec = detail.sessions.find(s => s.id === review.term_session_id)}
                {#if lec}<span class="review-session">{t('term.onLecture') || 'Lecture'} {lec.sort_order}: {lec.topic}</span>{/if}
              {/if}
            </div>
        ...
        {#if detail.review_count > detail.reviews.length}
          <a href="/term-reviews?id={encodeURIComponent(id)}" class="view-all">{t('term.viewAll')} ({detail.review_count}) →</a>
        {/if}
      {/if}
    </section>
    ...
    <section class="reviews-section">
      <h2>{t('term.learnerNotes') || 'Notes'}</h2>
      {#if getAuth()}
        <a href="/new?category=note&term_id={encodeURIComponent(id)}" class="write-review-btn">{t('term.writeNote') || 'Write a note'}</a>
      {/if}
      {#if detail.notes.length === 0}
        <p class="meta">{t('term.noNotes') || 'No notes yet.'}</p>
      {:else}
        {#each detail.notes as note}
          <a href="/article?uri={encodeURIComponent(note.at_uri)}" class="review-card">
            <div class="review-header">
              ...
              {#if note.term_session_id}
                {@const lec = detail.sessions.find(s => s.id === note.term_session_id)}
                {#if lec}<span class="review-session">{t('term.onLecture') || 'Lecture'} {lec.sort_order}: {lec.topic}</span>{/if}
              {/if}
            </div>
        ...
        {#if detail.note_count > detail.notes.length}
          <a href="/term-notes?id={encodeURIComponent(id)}" class="view-all">{t('term.viewAll')} ({detail.note_count}) →</a>
        {/if}
      {/if}
    </section>
  </div>
...
<style>
  .term-page { max-width: 100%; }
  .term-header { display: flex; gap: 32px; margin-bottom: 32px; padding-bottom: 24px; border-bottom: 1px solid var(--border); }
  .header-main { flex: 1; min-width: 0; }
  .header-side { width: 260px; flex-shrink: 0; }
  .term-code { font-size: 13px; font-weight: 600; color: var(--accent); background: rgba(95,155,101,0.1); padding: 3px 10px; border-radius: 3px; display: inline-block; margin-bottom: 8px; }
  .term-title { font-family: var(--font-serif); font-size: 2rem; font-weight: 400; margin: 0 0 8px; line-height: 1.3; }
  .term-institution { font-size: 14px; color: var(--text-secondary); margin: 4px 0; }
  .term-semester { font-size: 13px; color: var(--text-hint); margin: 2px 0; }
  .term-siblings { display: flex; flex-wrap: wrap; gap: 6px; align-items: center; margin: 8px 0 4px; font-size: 12px; }
  ...
  .term-authors { font-size: 14px; color: var(--text-secondary); margin: 6px 0; }
  .term-authors a { color: var(--text-primary); text-decoration: none; }
  .term-authors a:hover { color: var(--accent); }
  .term-desc { font-size: 14px; color: var(--text-secondary); line-height: 1.6; margin: 12px 0; }
  .term-tags { display: flex; flex-wrap: wrap; gap: 6px; margin: 12px 0; align-items: center; }
  .term-tag { display: inline-flex; align-items: center; gap: 3px; font-size: 12px; padding: 3px 10px; border-radius: 3px; background: rgba(95,155,101,0.1); color: var(--accent); transition: background 0.15s; }
  .term-tag a { color: inherit; text-decoration: none; }
  .term-tag:hover { background: rgba(95,155,101,0.2); }
  ...
  .term-tag-add { position: relative; display: inline-flex; gap: 4px; }
  ...
  .term-meta-box { font-size: 12px; color: var(--text-hint); display: flex; flex-direction: column; gap: 4px; }
  ...
  .term-resources { margin-bottom: 20px; }
  .term-resources h3 { font-family: var(--font-serif); font-weight: 400; font-size: 0.95rem; margin: 0 0 8px; color: var(--text-secondary); }
