...
DELETE FROM course_session_prereqs a USING (
    SELECT ctid FROM (
        SELECT ctid, ROW_NUMBER() OVER (PARTITION BY course_session_id, group_id, prereq_type ORDER BY tag_id) AS rn
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_course_session_tags_group
    ON course_session_tags (course_session_id, group_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_course_session_prereqs_group
    ON course_session_prereqs (course_session_id, group_id, prereq_type);
